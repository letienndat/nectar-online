...
class AccountService {
    ...
    func fetchProfile(token: String?, completion: @escaping (Result<Account ,Error>) -> Void) {
        ...
        let task = self.session.dataTask(with: request) { data, response, error in
            ...
            do {
                ...
                let response = try coder.decode(Response<Account>.self, from: data)
    ...
    func logout(token: String?, completion: @escaping (Result<Bool ,Error>) -> Void) {
        guard let token = token else {
            AppConfig.isLogin = false
            completion(.failure(NSError(domain: "", code: 401, userInfo: [NSLocalizedDescriptionKey: "Token is empty"])))
            return
